      ...
      <section className="container-page py-16 md:py-20">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="h-11 w-full rounded-md border border-border bg-background pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-ring"
          />
        </div>
        ...
        <ul className="mt-10 divide-y divide-border rounded-xl border border-border bg-card">
          {items.map((p) => (
            <li key={p.title} className="flex flex-wrap items-center gap-4 p-5 md:p-6">
              ...
              <a
                href={p.href}
                className="inline-flex items-center gap-1.5 rounded-md border border-input px-3.5 py-2 text-sm font-semibold hover:bg-muted"
              >
          ...
          {items.length === 0 && (
            <li className="p-6 text-muted-foreground">Aucun résultat.</li>
          )}
        </ul>
      </section>
    </>
